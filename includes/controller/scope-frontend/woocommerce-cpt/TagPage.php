...
class TagPage{
    ...
    const WC_TAG_PAGE_STYLE_FILENAME = 'tag-page.css';
    const WC_TAG_PAGE_STYLE_HANDLER = 'wc-tag-page-style';

    const WC_TAG_PAGE_SCRIPT_FILENAME = 'tag-page.js';
    const WC_TAG_PAGE_SCRIPT_HANDLER = 'wc-tag-page-script';

    public static $WC_TAG_PAGE_STYLE_PATH;
    public static $WC_TAG_PAGE_SCRIPT_PATH;
    ...
    public function add_Extra_Resources_to_WC_Tag_Pages(){
        ...
        add_action( 'wp_enqueue_scripts', [ $this,'enqueue_Extra_Resources_to_WC_Tag_Pages' ] );
    }//add_Extra_Resources_to_WC_Product_Pages

    public function register_Extra_Resources_to_WC_Tag_Pages(){
        /** 2.1. Register the custom styles */
        $css_version =  file_exists( self::$WC_TAG_PAGE_STYLE_PATH ) ? filemtime( self::$WC_TAG_PAGE_STYLE_PATH ) : false;
        wp_register_style( self::WC_TAG_PAGE_STYLE_HANDLER, self::$WC_TAG_PAGE_STYLE_PATH, array(), $css_version, 'all' );

        /** 2.2. Register the custom scripts */
        $js_version = file_exists( self::$WC_TAG_PAGE_SCRIPT_PATH ) ? filemtime( self::$WC_TAG_PAGE_SCRIPT_PATH ) : false;
        wp_register_script( self::WC_TAG_PAGE_SCRIPT_HANDLER, self::$WC_TAG_PAGE_SCRIPT_PATH, array(), $js_version, true );
    }//register_Extra_Resources_to_WC_Product_pages

    public function enqueue_Extra_Resources_to_WC_Tag_Pages(){

        if( is_product_tag() ){
            /** 2.1. Register the custom styles */
            $css_version =  file_exists( self::$WC_TAG_PAGE_STYLE_PATH ) ? filemtime( self::$WC_TAG_PAGE_STYLE_PATH ) : false;
            wp_enqueue_style( self::WC_TAG_PAGE_STYLE_HANDLER, self::$WC_TAG_PAGE_STYLE_PATH, array(), $css_version, 'all' );

            /** 2.2. Register the custom scripts */
            $js_version = file_exists( self::$WC_TAG_PAGE_SCRIPT_PATH ) ? filemtime( self::$WC_TAG_PAGE_SCRIPT_PATH ) : false;
            wp_enqueue_script( self::WC_TAG_PAGE_SCRIPT_HANDLER, self::$WC_TAG_PAGE_SCRIPT_PATH, array(), $js_version, true );
        }
    ...
    public function register(){
        ...
        $this->add_Extra_Resources_to_WC_Tag_Pages();
