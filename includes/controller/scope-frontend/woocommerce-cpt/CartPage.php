...
class CartPage{
    ...
    const WC_CART_PAGE_STYLE_FILENAME = 'cart-page.css';
    const WC_CART_PAGE_STYLE_HANDLER = 'wc-custom-cart-page-style';

    const WC_CART_PAGE_SCRIPT_FILENAME = 'cart-page.js';
    const WC_CART_PAGE_SCRIPT_HANDLER = 'wc-custom-cart-page-script';

    public static $WC_CART_PAGE_STYLE_PATH;
    public static $WC_CART_PAGE_SCRIPT_PATH;
    ...
    public function set_WooCommerce_Extra_Params(){
        ...
        self::$WC_CART_PAGE_STYLE_PATH = sprintf( 
            '%s%s%s%s', 
            PluginProperties::$PLUGIN_URL, 
            PluginProperties::RESOURCES_FRONTEND_ROOT_DIR,
            PluginProperties::CSS_ROOT_DIR.'woocommerce-cpt/', 
            self::WC_CART_PAGE_STYLE_FILENAME
        );
        
        self::$WC_CART_PAGE_SCRIPT_PATH = sprintf( 
            '%s%s%s%s', 
            PluginProperties::$PLUGIN_URL, 
            PluginProperties::RESOURCES_FRONTEND_ROOT_DIR,
            PluginProperties::JS_ROOT_DIR.'woocommerce-cpt/',  
            self::WC_CART_PAGE_SCRIPT_FILENAME
        );       

    }//set_WooCommerce_Extra_Params

    public function add_Extra_Resources_to_WC_Cart_Page(){
        /** 1. Add customizing activities after all plugins are loaded */
        // Hook after_setup_theme seems invalid
        // add_action( 'after_setup_theme', [ $this, 'register_Extra_Resources_to_WC_Cart_Page'], 100 );//OK
        // $this->register_Extra_Resources_to_WC_Cart_Page();
        ...
        // add_action( 'template_redirect', [ $this,'enqueue_Extra_Resources_to_WC_Cart_Page' ] );//OK
        add_action( 'wp_enqueue_scripts', [ $this,'enqueue_Extra_Resources_to_WC_Cart_Page' ] );
    }//add_Extra_Resources_to_WC_Product_Pages

    public function register_Extra_Resources_to_WC_Cart_Page(){
        /** 2.1. Register the custom styles */
        $css_version =  file_exists( self::$WC_CART_PAGE_STYLE_PATH ) ? filemtime( self::$WC_CART_PAGE_STYLE_PATH ) : false;
        wp_register_style( self::WC_CART_PAGE_STYLE_HANDLER, self::$WC_CART_PAGE_STYLE_PATH, array(), $css_version, 'all' );

        /** 2.2. Register the custom scripts */
        $js_version = file_exists( self::$WC_CART_PAGE_SCRIPT_PATH ) ? filemtime( self::$WC_CART_PAGE_SCRIPT_PATH ) : false;
        wp_register_script( self::WC_CART_PAGE_SCRIPT_HANDLER, self::$WC_CART_PAGE_SCRIPT_PATH, array(), $js_version, true );
    }//register_Extra_Resources_to_WC_Product_pages

    public function enqueue_Extra_Resources_to_WC_Cart_Page(){
        // $this->localDebugger->write_log_general( is_cart() );

        if( is_cart() ){
            /** 2.1. Register the custom styles */
            $css_version =  file_exists( self::$WC_CART_PAGE_STYLE_PATH ) ? filemtime( self::$WC_CART_PAGE_STYLE_PATH ) : false;
            wp_enqueue_style( self::WC_CART_PAGE_STYLE_HANDLER, self::$WC_CART_PAGE_STYLE_PATH, array(), $css_version, 'all' );

            /** 2.2. Register the custom scripts */
            $js_version = file_exists( self::$WC_CART_PAGE_SCRIPT_PATH ) ? filemtime( self::$WC_CART_PAGE_SCRIPT_PATH ) : false;
            wp_enqueue_script( self::WC_CART_PAGE_SCRIPT_HANDLER, self::$WC_CART_PAGE_SCRIPT_PATH, array(), $js_version, true );
        }
    ...
    public function register(){
        ...
        $this->add_Extra_Resources_to_WC_Cart_Page();
